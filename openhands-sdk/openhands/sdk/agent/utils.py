...
def _build_ripgrep_terminal_command(
    ...
    command_parts = ["rg", "-n", "-i", pattern,
                     search_path, "--sortr=modified"]
    if include:
...
def _build_python_grep_terminal_command(
    ...
    command_parts = ["python", "-c",
                     f"exec({_GREP_FALLBACK_SCRIPT!r})", pattern]
...
def _build_grep_terminal_command(arguments: dict[str, Any]) -> str | None:
    ...
    include_pattern = include if isinstance(
        include, str) and include.strip() else None
...
def make_llm_completion(
    ...
    """
    # 新一代统一接口，支持推理、多模态、Tool Calling 等更多能力
